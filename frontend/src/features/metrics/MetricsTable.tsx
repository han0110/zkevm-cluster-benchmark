...
import { buildGpuColumns, runColumn } from '@/components/common/gpuColumns';
...
export function MetricsTable({
  ...
  activeKey?: string;
  // Reports the displayed rows so the page can step its arrow keys through what the table shows.
  onVisibleRowsChange?: (rows: MetricRow[]) => void;
}) {
  const bench = useBench();
  const search = useRunSearch();

  // Stable row and column identities so the displayed-rows notification fires only on a real reorder
  // rather than every render.
  const rows = useMemo(() => buildMetricRows(bench), [bench]);
  const columns = useMemo<DataColumn<MetricRow>[]>(() => {
    ...
    return nodeCol ? [nodeCol, ...runColumns, ...rest] : [...runColumns, ...rest];
  }, [rows, search]);
